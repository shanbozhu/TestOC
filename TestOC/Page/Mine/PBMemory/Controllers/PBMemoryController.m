...
/**
 语言:
 1.编译型语言:c/c++、oc、swift等.编译成机器语言,操作系统可以直接执行可执行文件.程序自上而下逐行编译,主函数开始执行.
 2.解释型语言:java、python、javascript、shell等.解释成中间语言,操作系统需要安装对应虚拟机执行可执行文件.程序自上而下逐行编译执行.
 
 编辑 --> 编译/解释 --> 执行 --> 调试
 IDE(集成开发环境):
 1.代码提示
 2.断点调试(单步执行、单步执行进入、执行完当前函数、继续执行)
...
/**
 ...
 算法:具体使用场景.网络 + UI控件 + 多线程 + 数据库
 
 字符:数字、符号、英文字母
 标识符(变量名、函数名):数字、下划线、英文字母,且开头只能是下划线、英文字母
 ...
 对象类型:string(字符串)、dictionary(字典:键值对组合,键遍历)、arrar(数组:数据组合,索引遍历.又叫list,列表)、set(集合:无序不重复数据组合)、tuple(元组)、struct(结构体:数据组合,点语法遍历)
 */
