...
/**
 浏览器地址栏发请求（ua此时可以放在请求头cookie里）给后端（后端此时可以从请求头cookie里拿到ua，这里的ua是给后端使用的），后端返回页面内容（纯字符串：html css js）给浏览器。（此时是不同浏览器同一套页面内容，然后浏览器执行同一套js。当然也可以此时就不同浏览器不同页面内容，然后浏览器执行不同js，但一般不这么做。）
 浏览器执行js可以读取浏览器本身的ua（这里的ua是给前端js使用的），根据不同的ua发不同的请求（或执行其他操作）返回不同的json，浏览器执行js根据返回的json操作DOM（此时不同ua的浏览器里就可以显示不同样式）
 */

/**
 文案在前端代码写死，反正前端代码也是在线拉取的。不过要要改文案的话，需要前端重新上线(部署到服务器），而不是像后端可以开个配置平台直接配置，如果后端代码写死也需要后端重新上线。
