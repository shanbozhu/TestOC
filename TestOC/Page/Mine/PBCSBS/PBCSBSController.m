...
/**
 浏览器地址栏发请求（ua此时可以放在请求头cookie里）给后端（后端此时可以从请求头cookie里拿到ua，这里的ua是给后端使用的），后端返回页面内容（纯字符串：html css js）给浏览器。（此时是返回给不同浏览器同一套页面内容，然后浏览器执行同一套js。当然也可以此时就返回给不同浏览器不同页面内容，然后浏览器执行不同js，但一般不这么做。）
