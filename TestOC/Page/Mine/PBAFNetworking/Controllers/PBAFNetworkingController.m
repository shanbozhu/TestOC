...
/**
 ...
 一、应用层协议（请求）：
 1. 基于tcp的应用层协议：http（http://、https://）、websocket（ws://、wss://）、ftp（ftp://）
 1.1 http1.0：默认使用短连接
 1.2 http1.1：默认使用长连接，超时断开
 1.3 http2.0：默认使用长连接，超时断开，一个连接上可以并发请求
 1.4 websocket：默认使用长连接
 ...
 3. socket：基于[传输层协议]对外暴露的接口（API）。应用层协议都是调用socket接口实现的
 
 三、网络层协议：ip
 
 四、常用网络框架：
 http：使用AFNetworking
 websocket：使用SocketRocket
 */
...
/**
 https网络请求
 方式：GET（没有请求体）、POST
 请求：主机（host）、端口号（port）、请求路径（path）、请求参数（query）、请求头、请求体
 响应：响应头、响应体
 */
