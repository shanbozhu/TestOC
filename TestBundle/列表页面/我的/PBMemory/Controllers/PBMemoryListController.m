...
- (void)viewDidLoad {
    ...
    // 2.指向该对象的强指针数为0,对象被释放.即retainCount = 0时,对象被释放
    
    PBMemoryList *testList = [[PBMemoryList alloc]init];
    
    PBMemoryList *testList1 = testList; // 执行此语句会自动将对象的强指针数加1,相当于MRC下执行 PBMemoryList *testList1 = [testList retain];
    
    testList = nil; // 执行此语句会自动将对象的强指针数减1,相当于MRC下执行 PBMemoryList *testList1 = [testList release];
    testList1 = nil;
